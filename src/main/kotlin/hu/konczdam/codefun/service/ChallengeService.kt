...
class ChallengeService {
    ...
    fun modifyChallenge(challengeId: Long, challenge: Challenge): Challenge {
        ...
        val removedChallenges: MutableList<ChallengeTest> = mutableListOf()
        val idsOfChallengesModified = challenge.challengeTests.map {it.id}.filterNotNull()
        challengeFromDB.challengeTests.forEach {
            if (!idsOfChallengesModified.contains(it.id)) {
                removedChallenges.add(it)
            }
        }
        challengeFromDB.challengeTests.removeAll(removedChallenges)
        removedChallenges.forEach { challengeTestRepository.delete(it) }

        challenge.challengeTests.filter { it.id == null}.forEach {
            challengeFromDB.challengeTests.add(it)
            it.challenge = challengeFromDB
        }

        return challengeRepository.save(challengeFromDB)
    }
}